...
uint256 hashGenesisBlock("00000076d84c62af64353f3b59d8515191ee9f27e56d9c8422b1964aa6d167155x"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitBlock: starting difficulty is 1 / 2^12
...
{
    // BitBlock: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitBlock: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, bitblock - DarkGravity v2, written by Evan Duffield - evan@bitblock.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, bitblock - DarkGravity v3, written by Evan Duffield - evan@bitblock.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // BitBlock: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // BitBlock: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
void static BitBlockMiner(CWallet *pwallet)
{
    printf("BitBlockMiner started\n");
    ...
    {
        printf("BitBlockMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitBlockMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - BITBLOCK **********
    Copyright 2014, Bitblock Developers 
        eduffield - evan@bitblock.io
        InternetApe - kyle@bitblock.io
*/
